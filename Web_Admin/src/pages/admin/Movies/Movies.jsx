...
const getSaveErrorMessage = (err) => {
    if (err.code === 'ECONNABORTED') {
        return 'Lưu phim quá lâu, vui lòng kiểm tra APIService hoặc kết nối cơ sở dữ liệu';
    }

    if (err.response?.data?.message) return err.response.data.message;
    if (err.response?.data?.title) return err.response.data.title;
    if (err.message === 'Network Error') {
        return 'Không kết nối được APIService. Kiểm tra backend có đang chạy ở localhost:5001 không';
    }
